 ...
 * Support for templates from KPEntryTemplates <https://github.com/mitchcapper/KPEntryTemplates>


Installation
------------
Place KPEnhancedEntryView.plgx in your KeePass Plugins folder. KeePass also recommends using
dll files for additional security, to do so place all .dll files in the Plugins folder too.
...
Templates
---------
If KPEntryTemplates <https://github.com/mitchcapper/KPEntryTemplates> is used to create and
assign templates to entries these will be respected. KPEnhancedEntryView will show the fields
from the template for the entry, with the names and order assigned to them. Empty template
fields are also shown, unless the "Hide Empty Standard Fields" is enabled. Populated field
values which are not part of the template are shown after all template fields.
...
---------
v2.5.1
 Ensure populated standard fields are shown even if the template does not include them

v2.5.0
 Support for KPEntryTempates
 Follow new KeePass recommendation to distribute both plgx and dll files in release zip

v2.4.4
 Added compatibility with ColoredPassword <https://github.com/Rookiestyle/ColoredPassword>
  (v0.15.2 or later) when editing protected fields
